...
function styleIndicator(parent, el) {
  ...
  setInterval(() => {
    ...
    //         growVal = 0;
    //         play = true;
    //         el.style.boxShadow = `0 0 6px ${growVal}px #CCCCCC`;
    //         el.style.transform = `scale(${shrinkVal}, ${shrinkVal})`;
    //       }, 350);
    //     } else {
    //       growVal += 0.5;
    //       el.style.boxShadow = `0 0 6px ${growVal}px #CCCCCC`;
...
function addToDom(tourGuide) {
  for (const guide of tourGuide) {
    let indicator = document.createElement('div');
    indicator = styleIndicator(guide.el, indicator);
    document.getElementById('atlas-viewer').appendChild(indicator);
  }
}

export default {
  runMe: function(prefix) {
